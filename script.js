...
function normalizeStrain(strain) {
  ...
  return {
    ...strain,
    id: strain._id || strain.id || generateId(),
    _id: strain._id || strain.id || generateId(),
    // Ensure name exists (even if empty string)
...
function showDetail(strainId) {
  // Try to find by id first, then by _id (for API compatibility)
  const strain = appState.strains.find(
    (s) => s.id === strainId || s._id === strainId
  );
  if (!strain) {
    console.warn("⚠️ Strain not found:", strainId);
    return;
  }

  // Normalize the strain to ensure it has both id and _id
  appState.currentStrain = normalizeStrain(strain);
...
async function handleFormSubmit(e) {
  ...
  // Extract all form fields - ensure we get all values
  const strainData = {
    name: formData.get("name") || "",
    type: formData.get("type") || "Hybrid",
  ...
  if (!strainData.type || !strainData.type.trim()) {
    alert("Please select a strain type");
    return;
  }
  ...
  try {
    ...
    // Ensure data is saved before closing modal
    saveToLocalStorage();
    closeModal();
  } catch (error) {
    console.error("Error saving strain:", error);
    alert("Failed to save strain. Please try again.");
  }
}
...
async function addStrain(strainData) {
  ...
  // Use generateId() for consistent ID format
  const newStrain = normalizeStrain({
    ...strainData,
    _id: generateId(),
...
async function editStrain(id, strainData) {
  ...
  // Fallback to LocalStorage only
  const index = appState.strains.findIndex((s) => s.id === id || s._id === id);
  if (index === -1) return;

  // Preserve existing ID structure and update fields
  appState.strains[index] = normalizeStrain({
